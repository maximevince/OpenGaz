...
- **Online** — Title → _Play online_: one player creates a room and shares the code / invite link;
  ...
  step pulls a fresh snapshot from the host. The room has a chat: a pane in the lobby and while you
  wait for your turn, and during your turn press `T` (or `Enter`) to type a line, `Esc` to drop it;
  the last few lines hang in the corner for a moment, and the 💬 button opens the whole history.
  `/1` to `/9` send a canned line, in the Age of Empires tradition.
